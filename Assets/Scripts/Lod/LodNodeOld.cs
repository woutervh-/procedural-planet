...
public class LodNodeOld : IDisposable
{
    ...
    private LodNodeOld parent;
    private LodProperties lodProperties;
    private int lodLevel;
    private Vector3 origin;
    private Vector3 forward;
    private Vector3 right;
    public LodNodeOld[] children;
    private GameObject gameObject;
    private MeshFilter meshFilter;
    private MeshRenderer meshRenderer;
    private Mesh plane;

    public LodNodeOld(LodNodeOld parent, LodProperties lodProperties, int lodLevel, Vector3 origin, Vector3 forward, Vector3 right)
    {
        this.parent = parent;
        ...
        this.plane = new LodFace(LodNodeOld.CHUNK_RESOLUTION, lodProperties.heightGenerator, origin, lodProperties.up, forward, right).GenerateMesh();
    ...
    public void SplitRecursive()
    {
        ...
        {
            foreach (LodNodeOld child in this.children)
            {
    ...
    private void Split()
    {
        ...
        if (this.lodLevel == LodNodeOld.MAX_LOD_LEVEL)
        {
            return;
        }
        ...
        this.children = new LodNodeOld[4];
        this.children[0] = new LodNodeOld(this, this.lodProperties, this.lodLevel + 1, this.origin - childRight - childFoward, childFoward, childRight);
        this.children[1] = new LodNodeOld(this, this.lodProperties, this.lodLevel + 1, this.origin + childRight - childFoward, childFoward, childRight);
        this.children[2] = new LodNodeOld(this, this.lodProperties, this.lodLevel + 1, this.origin - childRight + childFoward, childFoward, childRight);
        this.children[3] = new LodNodeOld(this, this.lodProperties, this.lodLevel + 1, this.origin + childRight + childFoward, childFoward, childRight);
    }
    ...
    {
        float edgeLength = LodFace.GetEdgeLength(LodNodeOld.CHUNK_RESOLUTION);
        ...
        float tessellationFactor = edgeLengthAtDistance / LodNodeOld.DESIRED_EDGE_LENGTH;
    ...
    {
        if (lodLevel == LodNodeOld.MAX_LOD_LEVEL)
        {
            return 0f;
        }
        else
        {
            return Mathf.Pow(2f, LodNodeOld.MAX_LOD_LEVEL - lodLevel) / LodNodeOld.DETAIL_FACTOR;
        }
    }
    ...
    private static float LodLevelToMaxDistance(int lodLevel)
    {
        ...
        {
            return Mathf.Pow(2f, LodNodeOld.MAX_LOD_LEVEL - lodLevel + 1) / LodNodeOld.DETAIL_FACTOR;
        }
    }
}
